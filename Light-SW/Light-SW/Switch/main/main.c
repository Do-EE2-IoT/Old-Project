...
#define TAG "EXAMPLE"
#define BLE_MESH_DEVICE_NAME    "Button3" /*!< Device Advertising Name */ 
...
/* Configuring BLE Mesh network behaviors and setting up model publications for a node within the network */
static esp_ble_mesh_cfg_srv_t config_server = {
    .relay = ESP_BLE_MESH_RELAY_DISABLED, // Configuration for the relay feature of the node, which is disabled here. When enabled, the node can relay messages for other nodes.
    .beacon = ESP_BLE_MESH_BEACON_ENABLED, // Used for the provisioning and discovery of nodes.
...
void example_ble_mesh_send_gen_onoff_set(uint8_t button_state)
{
    ...
    /*Address sending*/
    common.ctx.addr = 0xC111;   /* to Light3, 4 */
...
void example_ble_mesh_send_gen_onoff_get(void)
{
    ...
    common.ctx.app_idx = store.app_idx; 
    /* Address sending */
    common.ctx.addr = 0x0010; // Replace with the server's unicast address
    common.ctx.send_ttl = 3;
    common.ctx.send_rel = false;
    common.msg_timeout = 0; // 0 indicates that timeout value from menuconfig will be used
    common.msg_role = ROLE_NODE;
                                               ...
                                               esp_ble_mesh_generic_client_cb_param_t *param)
{
    ...
    switch (event) {
    /*This event is triggered when the Generic Client receives a state response after sending a Get message.*/
    case ESP_BLE_MESH_GENERIC_CLIENT_GET_STATE_EVT:
        ...
        break;
    /*This event occurs when the Generic Client receives a state response after sending a Set message.*/
    case ESP_BLE_MESH_GENERIC_CLIENT_SET_STATE_EVT:
        ...
        break;
   /* When server send status by publish message */
    case ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT:
        ESP_LOGI(TAG, "ESP_BLE_MESH_GENERIC_CLIENT_PUBLISH_EVT");
        if (param->params->opcode == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_STATUS) {
        ESP_LOGI(TAG, "Received Generic OnOff Status, onoff %d", param->status_cb.onoff_status.present_onoff);
        state_button = param->status_cb.onoff_status.present_onoff;
        ESP_LOGI(TAG, "Button state: %d",state_button);
        break;
    default:
        break;
    }
