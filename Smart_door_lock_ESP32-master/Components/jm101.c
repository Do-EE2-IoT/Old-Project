...
static const char *jm101_TX = "jm101_TX";         //++ UART RX TAG

static void (*fingerprint_confirm_callback)();
...
{
    uint8_t received_confirmation_code = 0x01;
    ...
    if (rxBytes > 0)
    {
        ...
        ESP_LOGI("jm101_RX", "Read %d bytes: '%s'", rxBytes, received_package);
        ESP_LOG_BUFFER_HEXDUMP("jm101_RX_LOG", received_package, rxBytes, ESP_LOG_INFO);     //++ Dumps the response in HEX format
        jm101_response_parser(instruction_code, received_package);                       //++ Pass the received response to the parser function
...
{
    uint16_t result = 0;
    if(jm101_data[0] == '#')                                                             //++ Check whether to perform checksum for packet without extra data inputs
    {
        char length[4];
        sprintf(length,"%c%c",jm101_data[1],jm101_data[2]);
    ...
    else                                                                                //++ Check whether to perform checksum for packet with extra data inputs
    {
        ...
        {
            result = result + tx_cmd_data[i+6];
            if(i<sizeof(jm101_data))
            {
                result = result + jm101_data[i];
            }
        }
    }
...
uint8_t VfyPwd(char jm101_address[], char vfy_password[])
{
    ...
    {
        tx_cmd_data[i+2] = jm101_address[i];
    ...
    ESP_LOGI(jm101_TX, "Wrote %d bytes", txBytes);
    vTaskDelay(500 / portTICK_PERIOD_MS);

    confirmation_code = jm101_reponse(instruction_code);
...
{
    char tx_cmd_data[17] = {0xEF, 0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0x01, 0x00, 0x08, 0x32, 0x03, 0xFF, 0xFF, 0x00, 0x00};
    char check_sum_data[2] = {0x00, 0x00};
    uint8_t confirmation_code = 0;
...
{
    char tx_cmd_data[17] = {0xEF, 0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0x01, 0x00, 0x08, 0x31, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x3C};
    tx_cmd_data[10] = high_location;
    tx_cmd_data[11] = low_location;
    tx_cmd_data[16] = tx_cmd_data[16] + low_location;
    uint8_t confirmation_code = 0;
...
{
    char tx_cmd_data[17] = {0xEF, 0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0x01, 0x00, 0x08, 0x32, 0x02, 0xFF, 0xFF, 0x00, 0x00, 0x02, 0x3B};
    uint8_t confirmation_code = 0;
...
{
    char tx_cmd_data[13] = {0xEF, 0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0x01, 0x00, 0x04, 0x02, 0x01};
    char check_sum_data[2] = {0x00, 0x00};
    uint8_t confirmation_code = 0;
...
uint8_t Store(char jm101_address[], char buffer_id[], char page_id[])
{
    ...
    const int txBytes = uart_write_bytes(UART_NUM_1, tx_cmd_data, package_length);
    ESP_LOGI(jm101_TX, "Wrote %d bytes", txBytes);
    ESP_LOG_BUFFER_HEXDUMP("jm101_TX", tx_cmd_data, package_length, ESP_LOG_INFO);
    vTaskDelay(1000 / portTICK_PERIOD_MS);
...
void jm101_response_parser(char instruction_code[], uint8_t received_package[])
{
    ...
    if(instruction_code == 0x32 || instruction_code == 0x34)
    {
        if(confirmation_code == 0x00)
        {
            ...
            uint8_t parameter_code = 0;
            ESP_LOGI("GR_Auto", "(0x00H) READ COMPLETE");
            parameter_code = received_package[10];
            if (parameter_code == 0x00)
            {
                ESP_LOGI("GR_Auto", "(0x00H) COMMAND VALID");
            }
            else if (parameter_code == 0x01)
            {
                ESP_LOGI("GR_Auto", "(0x01H) ACQUIRE IMAGE");
            }
            else if (parameter_code == 0x05)
            {
                ESP_LOGI("GR_Auto", "(0x05H) COMPLETED COMPARE");
        ...
        {
            ESP_LOGE("GR_Auto/GR_Identify", "(0x07H) Failure to generate feature\n");
        }
        else if(confirmation_code == 0x09)
        {
            ESP_LOGE("GR_Auto/GR_Identify", "(0x09H) NO FINGERPRINT WERE FOUND\n");
        }
        else if(confirmation_code == 0x0B)
        {
            ESP_LOGE("GR_Auto/GR_Identify", "(0x0BH) ID NUMBER EXCEEDS RANGE\n");
        }
        else if(confirmation_code == 0x17)
        {
            ESP_LOGE("GR_Auto/GR_Identify", "(0x17H) RESIDUAL FINGERPRINT\n");
        }
        else if(confirmation_code == 0x23)
        {
            ESP_LOGE("GR_Auto/GR_Identify", "(0x23H) FINGERPRINT TEMPLATE IS EMPTY\n");
        }
        else if(confirmation_code == 0x24)
        {
            ESP_LOGE("GR_Auto/GR_Identify", "(0x24H) FINGERPRINT LIB IS EMPTY\n");
        }
        else if(confirmation_code == 0x26)
        {
            ESP_LOGE("GR_Auto/GR_Identify", "(0x26H) TIMEOUT\n");
        }
        else if(confirmation_code == 0x27)
        {
            ESP_LOGE("GR_Auto/GR_Identify", "(0x27H) INDICATES FINGERPRINT\n");
        }
    }
    ...
    if(instruction_code == 0x31)
    {
        if(confirmation_code == 0x00)
        {
            ESP_LOGI("AutoEnroll", "(0x00H) SUCCESSFUL\n");
        }
        else if(confirmation_code == 0x01)
        {
            ESP_LOGI("AutoEnroll", "(0x01H) ERROR\n");
        }
    }

    if(instruction_code == 0x32)
    {
        if(confirmation_code == 0x00)
        {
            ESP_LOGI("AutoEnroll", "(0x00H) SUCCESSFUL\n");
...
{
    fingerprint_confirm_callback = cb;
}